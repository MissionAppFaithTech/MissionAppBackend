# [ADR-0017]: Adoção do Bruno como Cliente HTTP Oficial do Repositório
...
A questão central é: **como garantir que as coleções de requisições HTTP evoluam em sincronia com o código, sejam revisáveis em Pull Requests, não imponham restrições de acesso por número de colaboradores e não criem dependência de nenhum fornecedor externo?**
...
- [Bruno — Site oficial e repositório open-source](https://www.usebruno.com/): referência principal do cliente HTTP adotado
- [Bruno — Ambientes e variáveis](https://docs.usebruno.com/secrets-management/overview): documentação de gestão de segredos e variáveis de ambiente nas coleções Bruno
- [Bruno — CLI (`@usebruno/cli`)](https://docs.usebruno.com/cli/overview): permite executar coleções Bruno em pipelines de CI/CD
- [Bruno — Formato de arquivo `.bru`](https://docs.usebruno.com/bru-lang/overview): formato nativo versionável em Git, que justifica a escolha em relação ao Postman
- [ADR-0008](./0008-uso-do-docker-para-ambiente-de-desenvolvimento-e-deploy.md): Docker que define o ambiente de desenvolvimento onde as coleções Bruno são usadas

# [ADR-0024]: Convenção de Documentação de Código com JSDoc
...
**Duas camadas de rigor dentro do próprio JSDoc:**
Nem todo `/** */` precisa das tags completas (`@param`, `@returns`, `@throws`, `@example`) — a decisão segue o mesmo princípio de "documente por que, não o quê", só que aplicado tag a tag:

- **Tags completas — sempre obrigatório em `app/services/`.** Todo método público de service leva `@param` em cada parâmetro, `@returns` descrevendo o que volta (mesmo quando `void` — diga qual é o efeito colateral), `@throws` quando lança, e `@example` com uma chamada real. Services concentram a lógica de negócio do projeto — é a camada mais reaproveitada e mais lida por quem não escreveu o código, então o custo de documentar sempre é menor que o custo de alguém reconstruir o contrato por leitura da implementação.
- **Tags completas — quando a função tem rigor real, em qualquer outra camada.** Fora de `app/services/`, use tags completas quando a função lança exceção (`@throws`), tem parâmetro cujo significado não é óbvio pela assinatura (`@param`), tem retorno cujo shape não é evidente pelo tipo (`@returns`), ou um exemplo de uso esclarece um formato de entrada/saída não trivial (`@example`). Funções em `app/utils/` quase sempre caem aqui — são chamadas por camadas que só veem a assinatura, sem contexto de onde a função foi escrita.
- **Prosa simples, sem tags** — reservado para `type`/`interface` em `app/types/` (não são funções — não há `@param`/`@returns`/`@throws` para aplicar) e para funções triviais fora de `app/services/` e `app/utils/`.

```typescript
// Tags completas — a função lança exceção e o parâmetro tem formato específico
/**
 * Converte strings de duração no formato "<N><s|m|h|d>" em segundos.
 *
 * @param value Duração no formato aceito por `SignJWT.setExpirationTime` do jose.
...
// Prosa simples — type sem função associada, nada para taggear
/**
 * Recorte de `HttpContext` para serviços que só precisam autenticar/gerar
 * tokens — evita passar o contexto inteiro quando só uma fração é usada (PoLP).
 */
export type AuthContext = HttpContext['auth']
```

**JSDoc é a única forma de documentar uma função, método ou classe. Nunca um bloco `//` multi-linha, e nunca as tags de comentário inline (`NOTE:`, `TODO:`, `WARNING:` etc — ver AGENTS.md) usadas como substituto de JSDoc.** As tags inline existem para uma observação pontual dentro do corpo de uma função, sobre uma linha ou bloco específico — não para descrever o que a função inteira faz ou por que ela existe. Se a informação é sobre a função/método/classe como um todo (contrato, invariante, efeito colateral), é `/** */`; se é sobre uma linha específica dentro da implementação, é `//` com a tag apropriada.

```typescript
// ❌ errado — documentação da função substituída por comentário `//`
// NOTE: fecha a conexão compartilhada, necessário para o processo encerrar sozinho
export async function closeSharedConnection(): Promise<void> { ... }

// ✅ correto — JSDoc documenta a função; a tag fica reservada para notas
// pontuais dentro do corpo, quando houver
/**
 * Fecha a conexão compartilhada — necessário em processos que devem encerrar
 * sozinhos, já que uma conexão aberta mantém o event loop vivo.
 */
export async function closeSharedConnection(): Promise<void> { ... }
```

Isso vale mesmo quando a camada não exige JSDoc por padrão (ex: Controllers) — nesses casos, a resposta não é "documentar com `//` no lugar do JSDoc proibido", é simplesmente não documentar a função como um todo. Uma tag `NOTE:` inline dentro do método continua permitida (ver exemplo de `passwordHash` em texto plano no `AccountController`), porque ela não está descrevendo a função — está anotando uma linha específica.
...
<table width="100%">
   ...
   <tbody>
      <tr>
         ...
         <td>✅ Obrigatório — tags completas</td>
         <td align="left">Camada de maior densidade lógica. JSDoc de classe sempre. Todo método público leva tags completas (<code>@param</code>, <code>@returns</code>, <code>@throws</code> quando lança, <code>@example</code>) — sem exceção por "assinatura óbvia", diferente das demais camadas. Métodos privados triviais (getters de uma linha, montagem de chave) ficam de fora — não são contrato para nenhum chamador externo.</td>
      </tr>
      <tr>
      ...
      <tr>
         <td><strong>Utils</strong> (<code>app/utils/</code>)</td>
         <td>✅ Obrigatório</td>
         <td align="left">Funções puras reutilizáveis por múltiplas camadas — o chamador não tem acesso ao contexto de onde a função foi escrita, só à assinatura. JSDoc em toda função exportada, com tags completas (<code>@param</code>, <code>@returns</code>, <code>@throws</code>, <code>@example</code>) sempre que a função lançar exceção, tiver parâmetro não óbvio ou formato de entrada/saída que se beneficie de um exemplo — ver seção "Duas camadas de rigor". Prosa sem tags só para funções triviais sem exceção e sem formato específico a esclarecer.</td>
      </tr>
      <tr>
         <td><strong>Types</strong> (<code>app/types/</code>)</td>
         <td>✅ Obrigatório</td>
         <td align="left">Todo <code>type</code>/<code>interface</code> exportado precisa de um comentário — JSDoc quando o tipo tem generics, múltiplos campos ou contrato não trivial; um <code>/** ... */</code> de uma linha quando é um alias simples. O comentário justifica por que o tipo existe como entidade separada (em vez de inline no ponto de uso) e/ou explica um campo cujo nome não basta. Um tipo sem essa justificativa é candidato a virar inline no único lugar que o usa.</td>
      </tr>
      <tr>
...
- [ADR-0023](./0023-estrategia-de-autenticacao-jwt-hibrido-com-revogacao-via-dragonflydb.md): invariantes de segurança do fluxo de autenticação — exemplo central de comportamento que justifica JSDoc
- [ADR-0020](./0020-convencoes-de-escrita-de-migracoes.md): convenção análoga para migrações de banco de dados
- [ADR-0003](./0003-arquitetura-de-diretorios-scaffolding-adonisjs-e-extensoes-do-projeto.md): pastas `app/constants/`, `app/utils/` e `app/types/` referenciadas nesta convenção — organização interna e critério de posicionamento

# [ADR-0019]: Adoção de UUID v7 como Estratégia de Chave Primária
...
A questão central é: **qual estratégia de geração de chave primária oferece performance de inserção previsível, ausência de enumeração, geração sem coordenação e tipo uniforme em todo o schema?**
...
- [RFC 9562 — UUID Version 7](https://www.rfc-editor.org/rfc/rfc9562#section-5.7): especificação oficial do UUID v7 com timestamp monotônico no prefixo
- [PostgreSQL — UUID type](https://www.postgresql.org/docs/current/datatype-uuid.html): documentação do tipo UUID nativo no PostgreSQL
- [uuid (npm) — v7 support](https://github.com/uuidjs/uuid): biblioteca usada para geração de UUID v7 no projeto
- [ADR-0004](./0004-adocao-do-postgresql-como-banco-de-dados.md): PostgreSQL como banco que armazenará as chaves UUID v7
- [ADR-0018](./0018-convencoes-de-escrita-de-migracoes.md): convenções de migrações que definem como a coluna de PK UUID é declarada

# [ADR-0028]: Arquitetura de Validators Reutilizáveis com VineJS
...
**O nome do validator (camada 3) descreve a ação/dado de negócio validado — nunca o verbo HTTP nem o nome do método do controller.** `loginValidator`, não `accessTokensStoreValidator`; `sessionValidator` (identifica uma sessão por `familyId`), não `sessionsDestroyValidator`. Isso mantém "um arquivo por handler" (seção anterior) como o caso comum, não uma regra rígida: quando dois handlers de controllers diferentes precisam validar exatamente o mesmo shape, o validator é reaproveitado em vez de duplicado. Caso real do projeto: `refreshValidator` (`app/validators/user/refresh.ts`) é importado tanto por `RefreshTokensController.store` quanto por `AccessTokensController.destroy` — os dois endpoints recebem `{ refreshToken }`, e nomear o validator pela ação ("renovar/apresentar um refresh token") em vez de pelo par verbo+controller é o que torna esse reuso possível sem duplicação nem acoplamento à estrutura de rotas.
...
- **Validators inline nos controllers:** Elimina a necessidade de arquivos separados de validator, mas acopla a lógica de validação ao controller (violando ADR-0025) e impede o reaproveitamento de schemas entre endpoints.
...
- [ADR-0025](./0025-convencoes-de-controllers-e-roteamento-rest.md): controllers sempre delegam validação a validators — nunca inline

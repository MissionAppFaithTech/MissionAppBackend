# [ADR-0014]: Adoção do Resend como Provedor de Email Transacional
...
A questão central é: **qual provedor de email transacional oferece a melhor combinação de entregabilidade, rastreabilidade via webhooks, DX para contribuidores e custo compatível com o modelo open-source do MissionApp?**
...
**Engine de template — Edge, não Nunjucks ou similar:**
O corpo HTML dos emails é renderizado com **Edge** (`edge.js`), o template engine oficial do AdonisJS, via `node ace add edge` e `message.htmlView('emails/<nome>', locals)` do próprio `@adonisjs/mail`. Templates vivem em `resources/views/emails/`. A escolha segue o mesmo princípio de mínimo acoplamento a ferramentas fora do ecossistema oficial já aplicado ao restante do projeto: Edge tem integração nativa com `@adonisjs/mail` (sem adapter, sem lib de template adicional), enquanto qualquer motor de terceiro (Nunjucks, Handlebars) exigiria configurar renderização manual por fora do `MessageComposeCallback`. Valores compartilhados por todos os templates (ex: nome da marca) ficam em `globals` na config (`config/mail.ts`), evitando repetição por template.

**Primeiro caso de uso real — recuperação de senha:**
O fluxo de "esqueci minha senha" (`PasswordResetService`, `app/services/auth/`) é o primeiro consumidor do `@adonisjs/mail` configurado por este ADR — reaproveita as colunas `recovery_password_token`/`recovery_password_token_expires_at` já existentes em `users` desde a migration original (dormentes até então), com token opaco de 256 bits (mesmo padrão de geração do refresh token, ver ADR-0023), TTL de 60 minutos, e template `resources/views/emails/forgot_password.edge`. Redefinir a senha por esse fluxo revoga todas as sessões ativas e reseta o bloqueio de força bruta (ver ADR-0023, seção "Bloqueio de Conta") — apresentar o token é prova de controle sobre o email cadastrado.

**Envio síncrono, não enfileirado — por ora:**
O envio ocorre de forma síncrona dentro do request (`mail.send()`), não via BullMQ. A resiliência contra indisponibilidade do Resend mencionada nas Consequências deste ADR pressupõe uma fila de envio — ainda não implementada; o projeto já usa BullMQ para outros fins (ADR-0012), mas cada fila hoje é um par Queue/Worker dedicado, iniciado via comando Ace próprio (ex: `commands/queue_refresh_tokens_cleanup.ts`), sem um dispatcher genérico de jobs sob demanda. Enfileirar o envio de email é uma extensão futura direta — não um redesenho — quando a resiliência a indisponibilidade do Resend se tornar prioridade.
...
O endpoint de webhook verificará a assinatura de cada requisição via cabeçalhos Svix (`svix-id`, `svix-timestamp`, `svix-signature`) antes de processar qualquer payload. O processamento do evento seguirá o padrão EDA estabelecido no [ADR-0012](./0012-adocao-de-arquitetura-orientada-a-eventos-com-bullmq.md): o endpoint HTTP apenas valida a assinatura e enfileira um Job no BullMQ — o Worker executa a lógica de atualização do usuário de forma assíncrona.
...
- **Dependência de serviço de terceiro:** O envio de emails transacionais depende da disponibilidade do Resend. Indisponibilidade do Resend impacta diretamente fluxos críticos como verificação de conta e recuperação de senha. A resiliência parcial é garantida pela fila BullMQ ([ADR-0012](./0012-adocao-de-arquitetura-orientada-a-eventos-com-bullmq.md)): emails enfileirados durante indisponibilidade serão reenviados quando o serviço se recuperar, mas o limite de tempo dos tokens de verificação/redefinição deve ser dimensionado considerando janelas razoáveis de indisponibilidade.
...
- [Resend — Documentação oficial](https://resend.com/docs): referência principal do provedor de email transacional adotado
- [Resend — Webhooks](https://resend.com/docs/webhooks/introduction): eventos de entrega e verificação de assinatura para rastreamento de emails
- [Resend — Pricing](https://resend.com/pricing): justifica a escolha pelo plano gratuito adequado ao estágio inicial do projeto
- [AdonisJS Mail](https://docs.adonisjs.com/guides/digging-deeper/mail#transport-configuration): transport e configuração do pacote de email do AdonisJS integrado ao Resend
- [AdonisJS — Edge.js](https://edgejs.dev/docs/introduction): engine de template usado para o corpo HTML dos emails
- [ADR-0012](./0012-adocao-de-arquitetura-orientada-a-eventos-com-bullmq.md): arquitetura de eventos que define como disparos de email são enfileirados via BullMQ
- [ADR-0023](./0023-estrategia-de-autenticacao-jwt-hibrido-com-revogacao-via-dragonflydb.md): estratégia de autenticação — contexto do fluxo de recuperação de senha, primeiro consumidor real deste ADR

# [ADR-0025]: Convenções de Controllers e Roteamento REST
...
### 7. Organização de arquivos por domínio

A regra dos cinco métodos padrão implica que operações não-CRUD de um mesmo domínio viram vários controllers de método único (ex: login, refresh, logout e gerenciamento de sessão são quatro controllers distintos do domínio `auth`). Para não deixar `app/controllers/` crescer como uma lista plana sem estrutura, controllers são organizados em subpastas por domínio — mesma convenção já usada em `app/enums/<domínio>/`, `app/validators/<domínio>/` e `app/models/filters/<domínio>/`:

```
app/controllers/
  auth/
    access_tokens_controller.ts
    refresh_tokens_controller.ts
...
O nome da subpasta segue o mesmo domínio documentado em `docs/api/v1/<domínio>/` (ADR-0027) — o critério de separação é idêntico: o que o endpoint faz (mecanismo de sessão vs. atributo de recurso), não em qual tabela o dado é persistido. A subpasta não afeta nome de rota nem o barrel gerado: o nome da rota vem do `.as()` dos grupos em `start/routes.ts` e do nome da classe do controller, não do caminho do arquivo — mover um controller entre pastas é reorganização pura, sem efeito em URLs ou testes que resolvem rotas pelo nome.
...
- Controllers de método único proliferam — o número de arquivos cresce mais rápido do que em projetos sem essa convenção. Mitigado pela organização em subpastas por domínio (seção "Organização de arquivos por domínio") — o crescimento continua, mas fica navegável.
...
- [ADR-0024](./0024-convencao-de-documentacao-de-codigo-com-jsdoc.md): convenção de documentação — controllers são camada sem JSDoc por serem intencionalmente finos
- [ADR-0003](./0003-arquitetura-de-diretorios-scaffolding-adonisjs-e-extensoes-do-projeto.md): generaliza a organização por subpasta de domínio introduzida aqui para `app/controllers/` e a estende a `app/services/` e `app/types/`

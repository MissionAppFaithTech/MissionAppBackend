...
- **Supabase:** O questionamento é recorrente — Supabase usa PostgreSQL por baixo, então "por que não usar diretamente?". O ponto crítico é que **Supabase não é um banco de dados — é um BaaS completo** (auth próprio via GoTrue/PostgREST, API REST autogerada, Realtime via websockets, Storage, Edge Functions). Usar Supabase como banco de dados seria carregar uma plataforma inteira para usar apenas uma fração dela, ignorando todas as outras features pelas quais haveria dependência ou custo.

  Descartado por três razões independentes, cada uma suficiente por si só:

  1. **Incompatível com os requisitos de autenticação do MissionApp.** O fluxo de aprovação de missionários exige que o cadastro passe por aprovação manual antes de liberar acesso — o Supabase Auth não tem esse conceito (todo `signUp` cria usuário ativo). O sistema de `jti` blocklist com `auth_version` no DragonflyDB para revogação imediata exigida pela LGPD não existe no GoTrue. A rotação de refresh tokens com `family_id` para detecção de roubo também não é padrão do GoTrue. O resultado seria usar o Supabase Auth para sign-up e sign-in, mas reimplementar tudo que importa para segurança em cima dele — o pior dos dois mundos.

  2. **Incompatível com o requisito de auto-hospedagem open-source.** A proposta é usar o PostgreSQL hospedado pelo Supabase via `DATABASE_URL`. Isso mantém o PostgreSQL como banco, mas exige que todo contribuidor crie uma conta Supabase para obter uma URL de conexão antes de rodar o projeto localmente. O MissionApp deve poder rodar completamente offline e sem dependência de conta em serviço externo. O free tier do Supabase pausa projetos inativos, o que quebraria o ambiente de desenvolvimento sem aviso. E se a Supabase alterar pricing, limites ou encerrar o serviço, todos os ambientes de desenvolvimento quebram simultaneamente — risco que `docker compose up` com PostgreSQL local elimina por completo.

  3. **Vendor lock-in contrário à diretriz do projeto.** A manutenção do projeto adota minimização de acoplamento a provedor como diretriz explícita. Supabase introduziria dependência em uma plataforma comercial que, ao contrário do PostgreSQL (padrão aberto com décadas de estabilidade), pode alterar seus termos, APIs ou modelo de negócio independentemente do MissionApp.

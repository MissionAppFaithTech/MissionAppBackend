# [ADR-0014]: Adoção do Snyk para Detecção e Correção de Vulnerabilidades
...
**Snyk como quality gate de segurança:**

O Snyk opera como _security quality gate_ no pipeline de CI/CD: uma barreira automática que impede código inseguro de avançar no fluxo de desenvolvimento. O conceito de quality gate de segurança é análogo ao quality gate de performance do k6 ([ADR-0028](./0028-adocao-do-k6-como-quality-gate-de-performance.md)) — define limites objetivos e mensuráveis que o código precisa satisfazer para ser aceito, eliminando a dependência de verificação manual de segurança em cada PR.

Para SCA (dependências), o threshold configurado é `--severity-threshold=high`: vulnerabilidades High e Critical bloqueiam o pipeline; Low e Medium geram alertas sem bloquear. Para SAST (Snyk Code), o mesmo critério se aplica — padrões de vulnerabilidade do OWASP Top 10 categorizados como High ou Critical impedem o merge.

Além do gate por PR, o workflow é executado semanalmente toda segunda-feira às 14h (horário de Brasília) de forma independente de atividade de desenvolvimento. Essa execução periódica é necessária porque novos CVEs são publicados continuamente — uma dependência sem alteração pode tornar-se vulnerável por uma descoberta externa. O scan semanal garante que o repositório seja avaliado contra o estado atual da base de dados de vulnerabilidades do Snyk, não apenas no momento em que cada PR foi aberto.

**Complementaridade com o Renovate ([ADR-0013](./0013-adocao-do-renovate-para-atualizacao-automatica-de-dependencias.md)):**
...
- **Semgrep CE (Community Edition) como SAST isolado:** Analisador estático open-source (LGPL-2.1), com suporte a rule packs públicos para TypeScript, Node.js, secrets e OWASP Top 10. Avaliado como alternativa para o papel de SAST. Descartado em favor do Snyk Code porque: (1) cobre apenas análise estática — não cobre SCA, container scanning nem IaC, exigindo combinação com outra ferramenta para atingir a mesma cobertura; (2) o Snyk Code já está presente no projeto como parte da plataforma Snyk adotada para SCA — consolidar SAST no mesmo workflow elimina um workflow separado e um conjunto adicional de dependências de CI; (3) a análise de alcançabilidade do Snyk Code prioriza alertas SAST com base no grafo de chamadas efetivo, reduzindo ruído de falsos positivos que o Semgrep reporta sem esse contexto. O Semgrep CE permanece como referência para projetos que necessitem de SAST sem dependência de plataforma SaaS e sem SCA integrado.
...
- **Vendor lock-in na plataforma Snyk:** A adoção do Snyk como plataforma centralizada de segurança introduz dependência em um único fornecedor SaaS. O risco é reconhecido e aceito com base em três argumentos:

  Primeiro, **o lock-in está confinado à camada de CI/CD, não ao código-fonte.** Os pontos de acoplamento são: o arquivo `security.yaml` (~30 linhas chamando `snyk/actions/*`), o secret `SNYK_TOKEN` e, futuramente, o arquivo `.snyk` de política de supressões. Nenhum desses pontos penetra no código da aplicação — não há imports, anotações, decorators ou referências ao Snyk em `app/`. Uma eventual migração consiste em substituir o workflow e reconfigurar a integração, não em refatorar código de produção. O custo de saída é operacional, não arquitetural.

  Segundo, **o formato de saída SARIF desacopla os resultados da ferramenta.** O Snyk publica findings via SARIF (Static Analysis Results Interchange Format — padrão OASIS), consumido pelo GitHub Code Scanning. Qualquer substituto que também produza SARIF — Trivy, Semgrep, CodeQL — integra-se ao mesmo GitHub Security tab sem alterar a forma como os resultados são surfaceados para o time. O pipeline de visibilidade de findings é agnóstico à ferramenta geradora.

  Terceiro, **a alternativa "sem lock-in" também tem lock-in — só que distribuído.** Substituir o Snyk por uma combinação de Trivy (container + SCA) + Semgrep CE (SAST) + `pnpm audit` (dependências) significa manter três ferramentas, três configurações de CI, três ciclos de atualização e perder análise de alcançabilidade — que é o diferencial que justifica o custo de concentração no Snyk. A ausência de vendor lock-in não é gratuita: seu custo é complexidade operacional e qualidade de análise reduzida. A migração torna-se racional apenas sob condições concretas e observáveis: alteração de pricing para repositórios open-source, indisponibilidade recorrente do serviço, ou surgimento de alternativa com análise de alcançabilidade equivalente a custo zero.
...
- [ADR-0013](./0013-adocao-do-renovate-para-atualizacao-automatica-de-dependencias.md): Renovate para atualização de dependências, complementar ao Snyk na gestão de segurança
- [ADR-0009](./0009-adocao-do-pnpm-como-gerenciador-de-pacotes.md): pnpm como gerenciador cujo lockfile é base do scanning do Snyk

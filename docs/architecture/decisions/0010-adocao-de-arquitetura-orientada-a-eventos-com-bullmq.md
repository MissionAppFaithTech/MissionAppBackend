# [ADR-0010]: Adoção de Arquitetura Orientada a Eventos com BullMQ para Operações Assíncronas
...
2. **Estágio 2 — Persistência e execução em background (BullMQ + DragonflyDB):** Um Listener, ao receber o evento, enfileira imediatamente um Job no BullMQ. O BullMQ persiste o Job no DragonflyDB (adotado no [ADR-0005](./0005-adocao-do-redis-como-cache-e-armazenamento-temporario.md)), garantindo durabilidade: o Job sobrevive a reinicializações do servidor. Um Worker isolado consome a fila e executa a operação pesada (indexação, email, compressão) no seu próprio ritmo, com retries automáticos configuráveis em caso de falha do serviço destino.
...
- **Reutilização da infraestrutura já adotada:** O DragonflyDB já está no stack ([ADR-0005](./0005-adocao-do-redis-como-cache-e-armazenamento-temporario.md)) como broker de filas. O BullMQ já é adotado como client de filas. Esse padrão não introduz nenhum novo serviço de infraestrutura — apenas formaliza como os componentes existentes se integram.
...
- [ADR-0005](./0005-adocao-do-redis-como-cache-e-armazenamento-temporario.md): DragonflyDB como broker de filas onde os jobs BullMQ são persistidos
- [ADR-0007](./0007-adocao-do-elasticsearch-como-mecanismo-de-busca.md): Elasticsearch como destino de jobs de indexação processados pelos workers

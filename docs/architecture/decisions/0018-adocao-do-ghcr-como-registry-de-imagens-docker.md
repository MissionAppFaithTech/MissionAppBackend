# [ADR-0018]: Adoção do GitHub Container Registry (GHCR) como Registry de Imagens Docker
...
- **Alinhamento com a imagem única ([ADR-0017](./0017-padrao-imagem-unica-multiplos-entrypoints-para-workers.md)):** O padrão de imagem única significa que há um único caminho de registry para gerenciar (`ghcr.io/org/missionapp-backend`). A API e todos os Workers são iniciados a partir dessa mesma tag — um push atualiza todos os processos do sistema distribuído de forma atômica, sem descompasso de versão entre serviços.
...
- [ADR-0010](./0010-uso-do-docker-para-ambiente-de-desenvolvimento-e-deploy.md): Docker como base da estratégia de containerização publicada no GHCR
- [ADR-0015](./0015-adocao-do-renovate-para-atualizacao-automatica-de-dependencias.md): Renovate para atualização automática de imagens base publicadas no GHCR
- [ADR-0016](./0016-adocao-do-snyk-para-deteccao-de-vulnerabilidades.md): Snyk para scanning das imagens publicadas no GHCR
- [ADR-0017](./0017-padrao-imagem-unica-multiplos-entrypoints-para-workers.md): padrão de imagem única publicada no GHCR com múltiplos entrypoints

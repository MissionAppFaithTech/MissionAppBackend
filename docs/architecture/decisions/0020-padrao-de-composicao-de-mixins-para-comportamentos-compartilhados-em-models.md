# [ADR-0020]: Padrão de Composição de Mixins para Comportamentos Compartilhados em Models
...
O AdonisJS já adota o padrão de mixin TypeScript com a função `compose()` internamente (ex: `withAuthFinder`).

A questão central é: **como padronizar a extensão de comportamentos compartilhados em Models de forma que seja composável, sem herança acidental, e alinhada com o idioma do framework?**
...
- [AdonisJS — Model mixins (`compose()`)](https://lucid.adonisjs.com/docs/model-factories#using-model-mixins): documentação do padrão `compose()` para composição de mixins em modelos Lucid
- [TypeScript — Mixin pattern](https://www.typescriptlang.org/docs/handbook/mixins.html): padrão de mixins em TypeScript que fundamenta a implementação adotada
- [ADR-0019](./0019-adocao-de-uuid-v7-como-estrategia-de-chave-primaria.md): UUID v7 como chave primária, implementado via mixin `WithPrimaryUuid`
- [ADR-0018](./0018-convencoes-de-escrita-de-migracoes.md): convenções de migrações, complementares às convenções de modelo definidas aqui

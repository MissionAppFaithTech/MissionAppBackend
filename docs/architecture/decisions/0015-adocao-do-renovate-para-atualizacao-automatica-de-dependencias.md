# [ADR-0015]: Adoção do Renovate para Atualização Automática de Dependências
...
O MissionApp Backend opera sobre um stack com múltiplas camadas de dependências versionadas: pacotes Node.js gerenciados pelo pnpm ([ADR-0011](./0011-adocao-do-pnpm-como-gerenciador-de-pacotes.md)), imagens Docker com tags fixas no `docker-compose.yaml` ([ADR-0010](./0010-uso-do-docker-para-ambiente-de-desenvolvimento-e-deploy.md)) e dependências de infraestrutura como Elasticsearch, DragonflyDB e MinIO. Cada uma dessas camadas tem um ciclo de release independente e acumula vulnerabilidades de segurança entre atualizações.
...
A questão central é: **como garantir que dependências Node.js, imagens Docker e lockfiles sejam mantidos atualizados de forma sistemática, com revisão humana antes do merge e sem depender da memória ou disponibilidade dos mantenedores?**
...
- **Suporte nativo ao pnpm e ao `pnpm-lock.yaml`:** O Renovate suporta o pnpm como gerenciador de pacotes de forma nativa, incluindo regeneração correta do `pnpm-lock.yaml` após atualizações de `package.json`. Isso é crítico para o projeto: a atualização de uma dependência sem atualização correspondente do lockfile violaria a garantia de reprodutibilidade estabelecida no [ADR-0011](./0011-adocao-do-pnpm-como-gerenciador-de-pacotes.md).

- **Manager nativo para Docker Compose:** O Renovate possui um manager dedicado ao `docker-compose.yaml` que detecta e atualiza tags de imagens automaticamente. Isso resolve diretamente o risco de acumulação de imagens com vulnerabilidades identificado no [ADR-0010](./0010-uso-do-docker-para-ambiente-de-desenvolvimento-e-deploy.md) — sem nenhum script customizado ou processo manual periódico.
...
- **Eliminação do risco de imagens Docker desatualizadas:** O manager do Renovate para Docker Compose cobre diretamente o risco identificado no [ADR-0010](./0010-uso-do-docker-para-ambiente-de-desenvolvimento-e-deploy.md) — tags de imagem são atualizadas via PR antes de acumularem vulnerabilidades, com changelog do release incluído na descrição do PR.

- **`pnpm-lock.yaml` sempre consistente com `package.json`:** Atualizações de pacotes regeneram o lockfile corretamente, mantendo a garantia de reprodutibilidade do [ADR-0011](./0011-adocao-do-pnpm-como-gerenciador-de-pacotes.md) sem intervenção manual.
...
- [Renovate — Documentação oficial](https://docs.renovatebot.com/): referência principal da ferramenta de atualização automática de dependências
- [Renovate — Manager Docker Compose](https://docs.renovatebot.com/modules/manager/docker-compose/): suporte a atualizações de imagens declaradas no docker-compose.yml
- [Renovate — Opções de configuração](https://docs.renovatebot.com/configuration-options/): referência das opções usadas no renovate.json do projeto
- [Renovate — Casos de uso](https://docs.renovatebot.com/getting-started/use-cases/): contexto de uso que justifica a adoção em projetos open-source
- [ADR-0010](./0010-uso-do-docker-para-ambiente-de-desenvolvimento-e-deploy.md): Docker como ambiente de desenvolvimento, cujas imagens também são gerenciadas pelo Renovate
- [ADR-0011](./0011-adocao-do-pnpm-como-gerenciador-de-pacotes.md): pnpm como gerenciador de pacotes cujas dependências são atualizadas pelo Renovate

...
**Recomendado para todos os sistemas (Linux, macOS e Windows): [mise](https://mise.jdx.dev/)**

```bash
# Linux / macOS
curl https://mise.run | sh

# Ativar no shell (adicione ao ~/.bashrc, ~/.zshrc ou equivalente)
eval "$(mise activate bash)"   # bash
eval "$(mise activate zsh)"    # zsh

# Na raiz do repositório, o mise lerá o .node-version automaticamente
mise install
```
...
# Ativar no PowerShell (adicione ao $PROFILE)
...
O mise detecta o `.node-version` ao entrar no diretório do projeto e ativa a versão correta automaticamente nas sessões seguintes.
